...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!$scheduledDate) {
    ...
    try {
      ...
      // Clear form
      $old = [
        'request_key'=>'','request_qty'=>'','send_key'=>'','send_qty'=>'',
...
.planet2{
  --r: 230px; width:24px; height:24px;
...
.card h2{ font-size: 1.25rem; margin-bottom: 0.6rem; color: rgba(255,255,255,0.92); letter-spacing: 0.02em; }
.helper{ margin-top: -0.2rem; margin-bottom: 0.9rem; color: rgba(255,255,255,0.68); font-size: 0.95rem; line-height: 1.35; }

label{ display:block; font-weight: 700; color: rgba(255,255,255,0.82); margin: 0.75rem 0 0.45rem; }
input, select{
  width:100%;
...
.meta{ grid-column: 1 / -1; background: var(--glass2); }
.actions{
  display:flex;
  gap: 0.9rem;
